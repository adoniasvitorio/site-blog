...
import { FaWordpress, FaWhatsapp, FaDev} from "react-icons/fa";
  ...
  const sIcons = Icons.socialIcons.map(icons => {
    return(
      <div>
        ...
        { icons.icon === "dev" ? (<Link to={icons.url} target="_blank"><FaDev/></Link>) : "" }
        { icons.icon === "whatsapp" ? (<Link to={icons.url} target="_blank"><FaWhatsapp/></Link>   ) : "" }
      </div>
    )
  })
	return (
